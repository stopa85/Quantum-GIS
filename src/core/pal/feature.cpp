      ...
      do {
         for (bbid=0;bbid<j;bbid++){
            ...
            if (layer->getArrangement() == P_FREE){
               enoughPlace = true;
               px = (box->x[0] + box->x[2]) / 2 - xrm*0.75;
               py = (box->y[0] + box->y[2]) / 2 - yrm*0.75;
               int i, j;
               for (rx=px,i=0; i<2; rx=rx+1.5*xrm,i++){
                  for (ry=py,j=0; j<2; ry=ry+1.5*yrm,j++){
                     // sould perhaps to tst with teh polyone insteand of the bbox
            ...
            else if (box->length > xrm && box->width > xrm){
               if (box->alpha <= M_PI / 4){   
                  alpha = box->alpha;
               }
               else{
            ...
            for (px=0;px<=box->width;px+=dx){
               for (py=0;py<=box->length;py+=dy){
...
int Feature::setPosition(double scale, LabelPosition ***lPos,
   ...
   // purge candidates which are outside the bbox
   // for line and polygon, shapes are reduced to box before...
   for (i=0;i<nbp;i++){
      if (!(*lPos)[i]->isIn (bbox) ) {
        rnbp--;
        (*lPos)[i]->cost = DBL_MAX;
