         ...
         for (ba=0;ba<4;ba++){ // for each bbox segement
            ...
            if (computeSegIntersection (shape->x[i], shape->y[i], shape->x[j], shape->y[j], 
               ...
               if (vabs(cross_product (bbx[ba], bby[ba], bbx[bb], bby[bb], shape->x[i], shape->y[i])) > 0.00001) {
                  // perhaps shape had  previuosly been splitted => new points generated by this operation  are on the bbox border => avoid to consider this cross as valid (stauts for those new points are == bboxSegNumber + 10 and 15 for bbox corner
...
void PointSet::splitPolygons (LinkedList<PointSet*> *shapes_toProcess,
   ...
   while (shapes_toProcess->size() > 0){
      ...
      if (retainedPt >= 0 && bestArea > 2*labelArea){ // there is a hole so we'll cut the shape in two new shape (only if hole area is bigger than twice labelArea)
#ifdef _DEBUG_FULL_
...
PointSet * PointSet::createProblemSpecificPointSet (double bbx[4], double bby[4], bool *outside, bool *inside){
   ...
   *inside = true;
   ...
   for (i=0;i<nbPoints;i++){
      shape->x[i] = this->x[i];
      shape->y[i] = this->y[i];
      if (isPointInPolygon (4, bbx, bby, x[i], y[i])){
         shape->status[i] = -1;
         *outside = false;
      }
      else{
...
CHullBox * PointSet::compute_chull_bbox (){
   ...
   for (alpha_d = 0; alpha_d<90;alpha_d++){
      ...
      bb[0]  = bbox[0]; bb[1]  = bbox[3]; // ax, ay

      bb[4]  = bbox[0]; bb[5]  = bbox[1]; // cx, cy

      bb[8]  = bbox[2]; bb[9]  = bbox[1]; // ex, ey

      bb[12] = bbox[2]; bb[13] = bbox[3]; // gx, gy


      bb[2]  = bb[0] + d1; bb[3]  = bb[1] + d2; // bx, by
      bb[6]  = bb[4] - d2; bb[7]  = bb[5] + d1; // dx, dy
      bb[10] = bb[8] - d1; bb[11] = bb[9] - d2; // fx, fy
      bb[14] = bb[12] + d2; bb[15] = bb[13] - d1; // hx, hy
...
double PointSet::getDistFromPolygon(double px, double py){
   if (nbPoints == 1){
   ...
   double *dist = new double[nbPoints];

   double *ext = new double[nbPoints];
   double *eyt = new double[nbPoints];
   double *fxt = new double[nbPoints];
   double *fyt = new double[nbPoints];
   ...
   isPointIn = isPointInPolygon (nbPoints, x, y, px, py);
