...
void Layer::registerFeature (const char *geom_id, double label_x, double label_y, PalGeometry *userGeom){
   int j;
...
void Layer::registerFeature (const char *geom_id, PalGeometry *userGeom){
   registerFeature (geom_id, 0, 0, userGeom);
}
...
// TODO set label size to 0 foreach feature ? (or translate from current values)
void Layer::setUnit(Units unit){
   if (unit == PIXEL || unit == METERS)
      label_unit = unit;
}
