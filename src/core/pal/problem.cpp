...
Problem::Problem() : nblp(0), all_nblp(0), nbft(0), displayAll(0), labelpositions(NULL), featStartId(NULL), featNbLp(NULL), inactiveCost(NULL), sol(NULL){
   ...
   bbox[3] = 0;
   featWrap = NULL;
   index = new RTree<LabelPosition*, double, 2, double>();
   index_sol = new RTree<LabelPosition*, double, 2, double>();
   index_subsol = NULL;
}
         ...
         for (j=0;j<featNbLp[i];j++){  // foreach candidate 
            if (!ok[featStartId[i]+j]){
               if ((lp = labelpositions[featStartId[i]+j])->nbOverlap == 0){ // if candidate has no overlap
                  ...
                  // 1) remove worse candidates from index
                  ...
                  for (k=j+1;k<featNbLp[i];k++){
                     ...
                     index->Search (amin, amax, removeOverlapCallback, (void*)lp2);
                     lp2->removeFromIndex(index);
                  }

                  //lp->removeFromIndex(index);
                  featNbLp[i] = j + 1;
                  break;
               }
            }
...
typedef struct {
   PriorityQueue *list;
   LabelPosition *lp;
   RTree <LabelPosition*, double, 2, double> *index;
} FalpContext;
...
void ignoreLabel (LabelPosition *lp, PriorityQueue *list, RTree <LabelPosition*, double, 2, double> *index){
   ...
   if (list->isIn(lp->id)){
      ...
      context->lp = lp;
      index->Search (amin, amax, falpCallback2, context);
   }
...
bool falpCallback1 (LabelPosition *lp, void *  ctx){
   LabelPosition *lp2 = ((FalpContext*)ctx)->lp;
   PriorityQueue *list = ((FalpContext*)ctx)->list;
   RTree <LabelPosition*, double, 2, double> *index = ((FalpContext*)ctx)->index;

   if (lp2->isInConflict (lp)){
      ignoreLabel (lp, list, index);
   }
   return true;
}
...
void Problem::init_sol_falp(){
   ...
   while (list->getSize() > 0){  // O (log size)
      ...
      for (i=featStartId[lp->probFeat];i<featStartId[lp->probFeat]+featNbLp[lp->probFeat];i++){
         ignoreLabel (labelpositions[i], list, index);
      ...
      context->lp = lp;
      index->Search(amin, amax, falpCallback1, (void*)context);
      index_sol->Insert (amin, amax, lp);
   }
      ...
      for (i=0;i<nbft;i++){ // forearch hidden feature
         if (sol->s[i] == -1){
            ...
            for (p=0;p<featNbLp[i];p++){
               ...
               index_sol->Search (amin, amax, countOverlapCallback, lp);
            ...
            retainedLabel->insertIntoIndex (index_sol);
...
void Problem::popmusic(){
   ...
   index_subsol = new RTree<LabelPosition*, double, 2, double>();
...
#ifdef _VERBOSE_
   sort_part_time = clock();
   std::cout << "   Sorting SubPart: " << (double)(sort_part_time - create_part_time)/(double)CLOCKS_PER_SEC << std::endl;
#endif

   //if (pal->initial_solution == FALP)
      init_sol_falp();
   //else
   //   init_sol_empty();
...
#ifdef _VERBOSE_
   init_sol_time = clock();
   std::cout << "   Compute initial solution: " << (double)(init_sol_time - sort_part_time)/(double)CLOCKS_PER_SEC;
#endif
   ...
   while (true){
      ...
      index_subsol->RemoveAll();

      for (i=0;i<current->subSize;i++){
         current->sol[i] = sol->s[current->sub[i]];
         if (current->sol[i] != -1){
            labelpositions[current->sol[i]]->insertIntoIndex (index_subsol);
         }
      }
     ...
     if (delta > EPSILON){
         ...
         for (i=current->borderSize;i<current->subSize;i++){
            ...
            if (sol->s[current->sub[i]] != -1){ 
               labelpositions[sol->s[current->sub[i]]]->removeFromIndex(index_sol);
            }

            sol->s[current->sub[i]] = current->sol[i];

            if (current->sol[i] != -1){
               labelpositions[current->sol[i]]->insertIntoIndex (index_sol);
            }
   ...
   std::cerr << r << "\t" << popit << "\t" << sol->cost <<  "\t" << (create_part_time - start_time)/(double)CLOCKS_PER_SEC <<   "\t" << (sort_part_time - create_part_time)/(double)CLOCKS_PER_SEC <<   "\t" << (init_sol_time - sort_part_time)/(double)CLOCKS_PER_SEC << "\t" << (search_time - init_sol_time)/(double)CLOCKS_PER_SEC << "\t" << (search_time - start_time)/(double)CLOCKS_PER_SEC <<   "\t" << nbActive << "\t" << (double)nbActive / (double)nbft;
...
SubPart * Problem::subPart(int r, int featseed, int *isIn){
   ...
   while (ri->size() < r && queue->size() > 0){
      ...
      for (i=featS;i<featS+p;i++){  // foreach candidat of feature 'id'
         ...
         context.lp = lp;
         index->Search (amin, amax, subPartCallback, (void*)&context);
      }
   }
...
double Problem::compute_feature_cost (SubPart *part, int feat_id, int label_id, int *nbOverlap){
   ...
   *nbOverlap = 0;
   ...
   if (label_id >= 0) { // is the feature displayed ?
      ...
      context.lp = lp;
      index_subsol->Search (amin, amax, countFullOverlapCallback, (void*)&context);
...
double Problem::popmusic_tabu(SubPart *part){
   ...
   while (it < stop_it && best_cost >= EPSILON){
      ...
      if (choosed_feat >= 0){
         ...
         if (old_label != -1)
            labelpositions[old_label]->removeFromIndex (index_subsol);
         ...
         if (old_label >= 0){
            ...
            index->Search (amin, amax, updateCandidatesCost, &context);
         }
         ...
         if (choosed_label >= 0){
            ...
            lp->insertIntoIndex (index_subsol);
         }
...
inline Chain *Problem::chain (SubPart *part, int seed){
   ...
   while (seed != -1){
         ...
         try {
            ...
            if (!(tmpsol[seed] == -1 && i == -1) && i+featStartId[subseed] != tmpsol[seed]){
               if (i != -1){
                  ...
                  index_subsol->Search (amin, amax, chainCallback, (void*)&context);
      ...
      if (next_seed == -1){
         ...
         if (et->old_label != -1){ 
            labelpositions[et->old_label]->removeFromIndex (index_subsol);
         }

         if (et->new_label != -1){
            labelpositions[et->new_label]->insertIntoIndex(index_subsol);
         }
   ...
   while (currentChain->size() > 0){
      ...
      if (et->new_label != -1){
         labelpositions[et->new_label]->removeFromIndex (index_subsol);
      }
         
      if (et->old_label != -1){ 
         labelpositions[et->old_label]->insertIntoIndex (index_subsol);
      } 
...
inline Chain *Problem::chain (int seed){
   ...
   while (seed != -1){
         ...
         try {
            ...
            if (!(tmpsol[seed] == -1 && i == -1) && i+featStartId[seed] != tmpsol[seed]){
               if (i != -1){ // new_label
                  ...
                  index_sol->Search (amin, amax, chainCallback, (void*)&context);
      ...
      if (next_seed == -1){
         ...
         if (et->old_label != -1){ 
            labelpositions[et->old_label]->removeFromIndex(index_sol);
         }

         if (et->new_label != -1){
            labelpositions[et->new_label]->insertIntoIndex(index_sol);
         }
   ...
   while (currentChain->size() > 0){
      ...
      if (et->new_label != -1){
         labelpositions[et->new_label]->removeFromIndex (index_sol);
      }

      if (et->old_label != -1){ 
         labelpositions[et->old_label]->insertIntoIndex (index_sol);
      } 
      ...
      if ((current_chain = chain (part, seed))){
         ...
         if (tabu_list[seed] < it || (cur_cost + current_chain->delta) - best_cost < 0.0) {
            ...
            for (i=0;i<current_chain->degree;i++){
               ...
               if (sol[fid] >= 0){
                  labelpositions[sol[fid]]->removeFromIndex (index_subsol);
               }
               sol[fid] = lid;
                
               if (sol[fid] >= 0){
                  labelpositions[lid]->insertIntoIndex(index_subsol);
               }
...
void Problem::check_solution(){
   ...
   index_sol->Search (amin, amax, checkCallback, (void*)list);
...
void Problem::chain_search(){
      ...
      if ((current_chain = chain (seed))){
         ...
         if (tabu_list[seed] < it || (cur_cost + current_chain->delta) - best_cost < -EPSILON) {
            ...
            for (i=0;i<current_chain->degree;i++){
               ...
               if (sol->s[fid] >= 0){
                  labelpositions[sol->s[fid]]->removeFromIndex (index_sol);
               }
               sol->s[fid] = lid;
                
               if (sol->s[fid] >= 0){
                  labelpositions[lid]->insertIntoIndex(index_sol);
               }
   ...
   for (i=0;i<nbft;i++)
      if (sol->s[i] != -1)
         labelpositions[sol->s[i]]->insertIntoIndex (index_sol);
   ...
   std::cerr << "\t" << it << "\t" << sol->cost <<  "\t" << (init_sol_time - start_time)/(double)CLOCKS_PER_SEC << "\t" << (search_time - init_sol_time)/(double)CLOCKS_PER_SEC << "\t" << (search_time - start_time)/(double)CLOCKS_PER_SEC <<   "\t" << nbActive << "\t" << (double)nbActive / (double)nbft;
#endif
...
void Problem::chain_search(){
      ...
      if (retainedChain && retainedChain->delta < - EPSILON){
         ...
         for (i=0;i<retainedChain->degree;i++){
            ...
            if (sol->s[fid] >= 0){
               ...
               context.lp = old;
               index->Search (amin, amax, nokCallback, &context);
            }
   ...
   std::cerr << "\t" << popit << "\t" << sol->cost <<  "\t" << (init_sol_time - start_time)/(double)CLOCKS_PER_SEC << "\t" << (search_time - init_sol_time)/(double)CLOCKS_PER_SEC << "\t" << (search_time - start_time)/(double)CLOCKS_PER_SEC <<   "\t" << nbActive << "\t" << (double)nbActive / (double)nbft;
#endif
      ...
      std::cout << "     chain accepted " << std::endl;
         ...
         for (i=0;i<retainedChain->degree;i++){
            ...
            if (sol[fid] >= 0){
               ...
               old->removeFromIndex(index_subsol);
...
void Problem::solution_cost(){
   ...
   for (i=0;i<nbft;i++){
      if (sol->s[i] == -1){
         ...
         context.lp = lp;
         index_sol->Search (amin, amax, countFullOverlapCallback, &context);
