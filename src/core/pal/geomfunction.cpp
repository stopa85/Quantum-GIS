...
bool computeSegIntersection (double x1, double y1, double x2, double y2,  // 1st (segment)
   ...
   //if (   cross_product (x1, y1, x2, y2, x3, y3) * cross_product (x1, y1, x2, y2, x4, y4) < 0.00001 
   //    && cross_product (x3, y3, x4, y4, x1, y1) * cross_product (x3, y3, x4, y4, x2, y2) < 0.00001)
  /*
   if (  cp1 * cp2 <= 0 && cp3 *cp4 <= 0){
      std::cout << x1 << "   " << y1 << "   " << x2 << "   " << y3 << "    " << x3 << "   " << y3 << "   "  << x4 << "   " << y4 << std::endl;
      ...
      std::cout << "     cp3*cp4 = " << cp3 * cp4 << std::endl << std::endl;;
   }*/
