...
Layer * Pal::addLayer (const char *lyrName, double min_scale, double max_scale, Arrangement arrangement, Units label_unit, double defaultPriority, bool obstacle, bool active, bool toLabel){
   ...
   for (std::list<Layer*>::iterator it = layers->begin(); it != layers->end();it++){
      if (strcmp ((*it)->name, lyrName) == 0){ // if layer already known
         ...
         lyr->arrangement = arrangement;
         lyr->defaultPriority = defaultPriority;
         lyr->obstacle = obstacle;
         lyr->active = active;
         lyr->toLabel = toLabel;
*/
         lyrsMutex->unlock();
...
typedef struct _featCbackCtx{
   ...
   RTree<PointSet*, double, 2, double> *ftIndex;
   double priority;
   double bbox_min[2];
   double bbox_max[2];
#ifdef _EXPORT_MAP_
...
/*
 * Callback function when looking for feat in RTree
 */
bool extractFeatCallback(Feature *ft_ptr, void *ctx){
   ...
   // all feature which are obstacle will be inserted into ftIndex
   if (context->layer->obstacle){
      ...
      context->ftIndex->Insert(min, max, ft_ptr);
#ifdef _EXPORT_MAP_
      svged = true;
      toSVGPath (ft_ptr->nbPoints, ft_ptr->type, ft_ptr->x, ft_ptr->y,
                 dpi , context->scale, 
   ...
   if (context->layer->toLabel){
      ...
      if (ft_ptr->label_x > 0.0000001 && ft_ptr->label_y > 0.0000001){
         ...
         for (i=0;i<ft_ptr->nbSelfObs;i++){
            ...
            context->ftIndex->Insert(min, max, ft_ptr->selfObs[i]);
         ...
         //if ((ft_ptr->type == geos::geom::GEOS_LINESTRING) 
         //  || ft_ptr->type == geos::geom::GEOS_POLYGON){
            double bbx[4], bby[4];
         ...
         while (feats->size() > 0){
            ...
            ft->nblp = ft->feature->setPosition (context->scale, &(ft->lPos), context->bbox_min, context->bbox_max, ft->shape
#ifdef _EXPORT_MAP_           
            , *context->svgmap
#endif
            );
 ...
 */
bool pruneLabelPositionCallback (PointSet *feat, void *ctx){

   LabelPosition *lp = ((PruneCtx*)ctx)->lp;
...
void releaseAllInIndex(RTree<PointSet*, double, 2, double> *ftIndex){   
   /*
   RTree<PointSet*, double, 2, double>::Iterator it;
   for (ftIndex->GetFirst(it) ; it.IsNotNull(); ftIndex->GetNext(it)){
      if (!(*it)->holeOf){
   ...
   */
   ...
   ftIndex->Search (amin, amax, releaseCallback, NULL);
}
...
Problem* Pal::extract(int nbLayers, char **layersName, double *layersFactor, double lambda_min, double phi_min, double lambda_max, double phi_max, double scale, std::ofstream *svgmap){
   // to store obstacles
   RTree<PointSet*, double, 2, double> *ftIndex = new RTree<PointSet*, double, 2, double>(); 
   ...
   /*
   proj->getXY(lambda_min, phi_min, &prob->bbox[0], &prob->bbox[1]);
   proj->getXY(lambda_max, phi_max, &prob->bbox[2], &prob->bbox[3]);
   ...
   */
   ...
   amin[0] = prob->bbox[0] = lambda_min;
   amin[1] = prob->bbox[1] = phi_min;
   amax[0] = prob->bbox[2] = lambda_max;
   amax[1] = prob->bbox[3] = phi_max;
   ...
   std::cout << "Feature:" << feat->feature->layer->name << "/" << feat->feature->uid << std::endl;
      ...
      // If candidates list is smaller than expected # of candidates
      if (max_p > feat->nblp)
         max_p = feat->nblp;  
      ...
      int stop;
      //if (feat->nblp > max_p){
      for (j=0;j<feat->nblp;j++){  /* foreach feat's label candidate */
         lp = feat->lPos[j];

#ifdef _DEBUG_FULL_
         std::cout << "  Prune" << std::endl;
         std::cout << "    lp->id: " << lp->id << std::endl;
         std::cout << "    feat->id: " << lp->feature->uid << std::endl;
         std::cout << "    layer->name: " << lp->feature->layer->name << std::endl;
#endif         
         //lp->cost = 0.0;
         ...
         //  confront label position with obstacle and increase his cose is case of conflits (label is overlapping an obstacle)

         pruneContext.lp = lp;

         if (lp->feature->nbSelfObs > 0){
            lp->feature->fetchCoordinates();
         }

#ifdef _DEBUG_FULL_
         std::cout << "rtree->Search(prune...)" << std::endl;
#endif
         ftIndex->Search (amin, amax, pruneLabelPositionCallback, (void*)&pruneContext);
...
#ifdef _DEBUG_FULL
      std::cout << "prune is finished" << std::endl;
#endif
      ...
      //if (feat->feature->type == geos::geom::GEOS_POLYGON && (feat->feature->layer->arrangement == P_FREE || feat->feature->layer->arrangement == P_HORIZ))
      if (feat->feature->type == GEOS_POLYGON && (feat->feature->layer->arrangement == P_FREE || feat->feature->layer->arrangement == P_HORIZ))
         setCost (stop, feat->lPos, max_p, ftIndex, bbx, bby);
   ...
   while (fFeats->size() > 0){ // foreach feature
      ...
      for (i=0;i<feat->nblp;i++,idlp++){  // foreach label candidate 
         ...
         prob->index->Search(amin, amax, countOverlapCallback, (void*)lp);
   ...
   std::cout << prob->nbOverlap << std::endl;
...
/*
void Pal::setProjection (const char *projStr){
   if (projection)
      delete[] projection;
   ...
   projection = new char[strlen (projStr) + 1];
   strcpy (projection, projStr);

   //projection = new char[3];
   //strcpy (projection, "no");
...
/*
void Pal::setInitialSolutionMethod (InitialSolutionMethod method){
   initial_solution = method;
}
*/
...
/*
const char * Pal::getProjection (){
   return projection;
}
*/

/*InitialSolutionMethod Pal::getInitialSolutionMethod (){
