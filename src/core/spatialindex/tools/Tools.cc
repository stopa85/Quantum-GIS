...
Tools::Random::Random(unsigned long seed, unsigned short xsubi0)
...
Tools::Random::~Random()
{
	...
	{
	case RGT_MERSENNE:
		delete[] reinterpret_cast<unsigned long*>(m_buffer);
		break;
	case RGT_DRAND48:
		delete[] reinterpret_cast<unsigned short*>(m_buffer);
		break;
	}
}
...
{
	m_buffer = new unsigned long[Tools::Random::MERS_N + 1];

	unsigned long& mti = *(reinterpret_cast<unsigned long*>(m_buffer));
	unsigned long* mt = reinterpret_cast<unsigned long*>(m_buffer) + 1;
	...
	assert(mti == *(reinterpret_cast<unsigned long*>(m_buffer)));

	m_architecture = Tools::System::getArchitecture();
}

void Tools::Random::initDrand(unsigned short xsubi0)
{
	unsigned short* pS = new unsigned short[3];
	pS[0] = xsubi0;
...
long Tools::Random::nextUniformLong()
{
	if (m_type == RGT_DRAND48)
	{
		unsigned short* xsubi = reinterpret_cast<unsigned short*>(m_buffer);
...
unsigned long Tools::Random::nextUniformUnsignedLong()
{
	return static_cast<unsigned long>(nextUniformLong());
}
...
unsigned long Tools::Random::nextUniformUnsignedLong(unsigned long low, unsigned long high)
{
	return low + static_cast<unsigned long>((high - low) * nextUniformDouble());
}

long long Tools::Random::nextUniformLongLong()
{
	return static_cast<long long>(nextUniformUnsignedLongLong());
}

unsigned long long Tools::Random::nextUniformUnsignedLongLong()
{
	assert(sizeof(unsigned long long) > sizeof(unsigned long));

	unsigned long long lh = static_cast<unsigned long long>(nextUniformLong());
	unsigned long long ll = static_cast<unsigned long long>(nextUniformLong());
	unsigned long long ret = (lh << (sizeof(long) * CHAR_BIT)) | ll;
	return ret;
}
...
{
	return low + static_cast<long long>((high - low) * nextUniformDouble());
}
...
{
	return low + static_cast<unsigned long long>((high - low) * nextUniformDouble());
}

short Tools::Random::nextUniformShort()
{
	return static_cast<short>(nextUniformUnsignedShort());
}

unsigned short Tools::Random::nextUniformUnsignedShort()
{
	return nextUniformUnsignedLong() >> ((sizeof(unsigned long) - sizeof(unsigned short)) * CHAR_BIT);
}
...
double Tools::Random::nextUniformDouble()
{
	...
	else if (m_type == RGT_MERSENNE)
	{
		...
		unsigned long r = nextUniformLong();
...
long Tools::Random::nextSkewedLong(long low, long high, Level p)
{
	return low + static_cast<long>(static_cast<double>(high - low) * nextSkewedDouble(p));
}
...
double Tools::Random::nextSkewedDouble(Level p)
{
	...
	//HsubV = 0.0;
	//for(i = 1; i <= V; i++) HsubV += 1.0 / std::pow((double) i, l);
...
unsigned long Tools::Random::getSize() const
{
	// the seed is enough.
	return sizeof(unsigned long);
}

unsigned long Tools::Random::getSeed() const
{
	return m_seed;
}

Tools::PRGZipf::PRGZipf(long min, long max, double s, Tools::Random* pRandom)
...
long Tools::PRGZipf::nextLong()
{
	...
	long ret = static_cast<long>(i - m_pLookupTable) + m_min;
...
Tools::UniversalHash::UniversalHash(unsigned long k)
 : m_k(k)
{
	if (m_k < 2)
		throw Tools::IllegalArgumentException(
	...
	Tools::Random r;
	m_a = new unsigned long long[m_k];
	for (unsigned long i = 0; i < m_k; i++)
		m_a[i] = r.nextUniformUnsignedLongLong(1ull, m_P);
}

Tools::UniversalHash::UniversalHash(Tools::Random& r, unsigned long k)
 : m_k(k)
{
	if (m_k < 2)
		throw Tools::IllegalArgumentException(
...
Tools::UniversalHash::UniversalHash(unsigned long a, unsigned long b)
 : m_k(2)
{
	m_a = new unsigned long long[m_k];
	m_a[0] = a;
...
Tools::UniversalHash::UniversalHash(const UniversalHash& h)
{
	m_k = h.m_k;
	m_a = new unsigned long long[m_k];
	memcpy(m_a, h.m_a, m_k * sizeof(unsigned long long));
}
...
Tools::UniversalHash::UniversalHash(const byte* data)
{
	...
	m_a = new unsigned long long[m_k];
	memcpy(m_a, p, m_k * sizeof(unsigned long long));
}
...
Tools::UniversalHash& Tools::UniversalHash::operator=(const UniversalHash& in)
{
	if (this != &in)
	{
		...
		memcpy(m_a, in.m_a, m_k * sizeof(unsigned long long));
	}
...
{
	unsigned long long r = m_a[0];
	unsigned long long xd = 1ull;

	for (unsigned long i = 1; i < m_k; i++)
	{
		xd = (xd * x) % m_P;
		r += (m_a[i] * xd) % m_P;
...
unsigned long Tools::UniversalHash::getSize() const
{
	return m_k * sizeof(unsigned long long) + sizeof(unsigned long);
}

void Tools::UniversalHash::getData(byte** buffer, unsigned long& length) const
{
	length = getSize();
	*buffer = new byte[length];
	...
	p += sizeof(unsigned long);
	memcpy(p, m_a, m_k * sizeof(unsigned long long));
	p += m_k * sizeof(unsigned long long);
